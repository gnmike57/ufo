...
class UITree:
    ...
    def _get_ui_tree(
        self, root: UIAWrapper, level: int = 0, self_root_rect: Optional[Any] = None
    ) -> Dict[str, Any]:
        """
        Get the UI tree.
        :param root: The root element of the UI tree.
        ...
        ui_tree = {
            ...
            "rectangle": {
                "left": elem_rect.left,
                "top": elem_rect.top,
                "right": elem_rect.right,
                "bottom": elem_rect.bottom,
            },
            "adjusted_rectangle": {
        ...
        if level < MAX_UI_TREE_DEPTH:
            for child in root.children():
                try:
                    ...
                    is_positive_area = (child_rect.right - child_rect.left) > 0 and (
                        child_rect.bottom - child_rect.top
                    ) > 0
                    if is_positive_area or len(child.children()) > 0:
                        ui_tree["children"].append(
                            self._get_ui_tree(child, level + 1, self_root_rect)
                        )
                except Exception as e:
